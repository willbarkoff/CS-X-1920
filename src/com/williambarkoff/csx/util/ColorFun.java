...
public class ColorFun {
    ...
    public static Color parseHex(String hex) throws InvalidHexException {
        if (hex.length() != 6 && hex.length() != 7) {
            throw new InvalidHexException("Invalid hex code");
        }

        if (hex.length() == 7) {
            hex = hex.substring(1); // get rid of #
        }

        String redS = hex.substring(0, 2);
        String greenS = hex.substring(2, 4);
        String blueS = hex.substring(4, 6);
    ...
    public static class InvalidHexException extends Exception {
        InvalidHexException(String message) {
            super(message);
        }
    }
}
